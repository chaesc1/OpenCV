...
#원본신호를 2개원소를 가질때까지 분리를 하기 때문에 원본 신호의 길이는 2의 지수승이 된다.
#하지만 영상이 반드시 2의 지승이 아니라는 문제가 있기때문에
#0 삽입기법으로 해결
#가로와 세로를 2의 지수승이 되게 키우고 나머지를 검은색 즉 0 으로 채우는 것이다.
def zeropadding(img):
    h, w = img.shape[:2]
    m = 1 << int(np.ceil(np.log2(h)))         # 2의 승수값을 로그를 취해서 계산
    n = 1 << int(np.ceil(np.log2(w)))         #계산후 올림처리후 shift연산을 통해 2의 지수승을 만든다.
